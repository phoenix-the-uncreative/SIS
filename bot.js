...
let anonCooldowned = [], anonChannel = [], anonSend = [];
...
bot.on('message', msg => {
	...
	database.connection.query(`SELECT * FROM \`${config.table}\` WHERE \`${config.column}\` = ${msg.author.id} and \`${config.column_ban}\` = 0`)
		.on('result', result => {
				if (result.length == 0) {
					msg.channel.send(
					...
					if (anonCooldowned[msg.channel.id] == (false || undefined)) {
						if (text == "анон старт") {
							...
							.send(`- Принято анонимное объявление от пользователя ${msg.author.tag} за ${`${new Date().toLocaleString()}.\n\n**Текст объявления**:\n`}${msg.content}`);
							msg.channel
							.send("Сообщение принято и отправлено. Следующее анонимное сообщение может быть отправлено только через час.\nСпасибо за использование услуги анонимной отправки сообщений.");
							anonCooldowned[msg.channel.id] = true;
							anonSend[msg.channel.id] = 0;
							setTimeout(() => anonCooldowned[msg.channel.id] = false, 3600000);
						} else msg.channel.send("Я реагирую только на команду \"анон старт\", а на другие темы разговаривать не умею. Пока.");
					} else msg.channel.send("Возможна отправка лишь одного анонимного сообщения в час. Пожалуйста, подождите.");
				};
			})
		.on('error', err => callback({ error: true, err: err }));
});
